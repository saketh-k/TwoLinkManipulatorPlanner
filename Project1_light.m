...
% need to redefine as theta values
label_location= [
                0.8    0.75;
                1.25 sqrt(3)/4;
                1.571  0;
...
all_thetas = [all_thetas' thetas4']';
toc

label_locationCartesian = [];
for i=1:length(label_location)
     [x,y] = thetaToXY(label_location(i,:), len1, len2);
     label_locationCartesian(i,1) = x;
     label_locationCartesian(i,2) = y;
end

animateArm(len1, len2, [all_thetas], obs, label_locationCartesian);
